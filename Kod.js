// 2019-06-18
// Ma7mtAuIo8ZTSGd_i5YpdCGhnh5TmGtYz
// https://developers.google.com/apps-script/guides/libraries

// TODO:
// Porównywanie dat - zrobić test
// Pobieranie danych w formie jesona.
...
function SqlAbstract(params){
  ...
  if (params && 'sheets' in params){
    for (var i in params.sheets){
      ...
      tables[sheet.getName()] = {sheet: sheet};
    }
  }
  ...
  this.insert = function(opt){
    ...
    if (isObject(opt.values)){
      appendRow(sheet, opt.values, cachedRows);
    }
...
//    }
    ...
    for (var n in opt.tables){
      ...
      this.createTable({ss: ss, table: table});
    }
  }
 ...
 *
 * @return {array} returns selected rows as array of objects (methods: get('columnName'), set({'columnName': newValue}))
 */
function selectData(sheet, where, groupBy, orderBy, fields, serializer, data){
  var spreadSheet = sheet.getParent();
  var timeZone = spreadSheet.getSpreadsheetTimeZone();

  return selectData_(sheet, where, groupBy, orderBy, false, fields, timeZone, serializer, data);
}
...
function selectDataRows(sheet, where, groupBy, orderBy, fields){
  var spreadSheet = sheet.getParent();
  var timeZone = spreadSheet.getSpreadsheetTimeZone();
  return selectData_(sheet, where, groupBy, orderBy, 'simple', fields, timeZone);
}

// Main function
function selectData_(sheet, where, groupBy, orderBy, responseType, fields, timeZone, serializer, data){
  // --- Functions needed ---
...
//  var headers = data.shift();
  ...
  for (var n = 1; n < dataLength; n++){
    ...
    if (fields && fields.length){
      rowData = filterFields(rowData, fields, columnIdByName, timeZone);
    }
      ...
      if (responseType != 'simple'){
        row = {
          ...
          get: function(colName){
            if (colName){
              if (serializer && colName in serializer && 'get' in serializer[colName]){
                return serializer[colName].get(this.data[columnIdByName[colName]]);
              }
              else {
                return this.data[columnIdByName[colName]];
              }
            }
      else {
        var hash = hashify(this.data, headers);
        
        if (serializer){
          for (var colName in hash){
            if (colName in serializer && 'get' in serializer[colName]){
              hash[colName] = serializer[colName].get(hash[colName]);
            }
          }
        }
        ...
        return hash;
          ...
          set: function(changes){
            for (var colName in changes){
              ...
              // Serialize
              if (serializer && colName in serializer && 'set' in serializer[colName]){
                colValue = serializer[colName].set(colValue);
              }
              
              sheet.getRange(this.rowNr, columnIdByName[colName] + 1).setValue(colValue);
              this.data[columnIdByName[colName]] = colValue;
            }
            return this;
          },
        };
  ...
  if (groupBy){
    if (orderBy){
      groupedDataMap(responseGrouped, function(nod){orderByColumns(nod, orderBy)});
    }
...
function filterFields(row, fields, columnIdByName, timeZone){
  var out = [];
...
function getFilterFunction(columnIdByName, where){
    ...
    for (var name in whereColumns){
      (function(){
        ...
        var comp = getComparisonFunction_(condition, function(a, b){return a == b;});
...
function getComparisonFunction_(condition, comp){
  ...
  if (condition && typeof condition === 'object' && condition.constructor === Object){
    ...
    else if (condValue && typeof condValue === 'object' && condValue.constructor === Date){
      return function(value){
        if (typeof value === 'object' && value.constructor === Date){
          return value.getTime() == condValue.getTime();
        }

        // return value == Utilities.formatDate(condValue, "GMT", "yyyy-MM-dd'T'HH:mm:ss'Z'");
        return false;
      };
    }
    else {
...
function updateRow(sheet, rowNr, changes){
  if (!sheet || !rowNr || !changes){
    throw 'ERROR: Parameters "sheet", "rowNr" and "changes" are needed!';
  }
  
  var data = sheet.getDataRange().getDisplayValues(); // runTime: 861 ms
  var headers = data.shift();
  ...
  for (var colName in changes){
    var colValue = changes[colName];
    sheet.getRange(rowNr, columnIdByName[colName] + 1).setValue(colValue);
  }
...
function appendRow(sheet, data, cachedRows){
  if (!sheet){
    throw 'ERROR: Parameters "sheet" is needed!';
  }
